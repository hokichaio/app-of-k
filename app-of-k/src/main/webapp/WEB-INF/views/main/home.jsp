	...
	<div id="wrapper">
		...
		<div class="areaLeft">
			...
			<c:forEach items="${ranking}" var="ranking">
				<li><img src="http://graph.facebook.com/<c:out value="${ranking.user.id}"/>/picture" align="middle"/>
				<br/><c:out value="${ranking.user.name}"/>：<c:out value="${ranking.score}"/>ポイント！</li><br/>
			</c:forEach>
			</ul>
		</div>
		<div class="areaRight">
			...
			<script language="javascript">
				if (AC_FL_RunContent == 0) {
					...
					AC_FL_RunContent(
						...
						'movie', '<%= request.getContextPath() %>/resources/game/water',
						'salign', ''
						); //end AC code
				}
			</script>
